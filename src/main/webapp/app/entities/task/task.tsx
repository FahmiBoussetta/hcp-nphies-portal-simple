  ...
  return (
    <div>
      <h2 id="task-heading" data-cy="TaskHeading">
        <Translate contentKey="hcpNphiesPortalSimpleApp.task.home.title">Tasks</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hcpNphiesPortalSimpleApp.task.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hcpNphiesPortalSimpleApp.task.home.createLabel">Create new Task</Translate>
          </Link>
        </div>
      </h2>
        ...
        {taskList && taskList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="hcpNphiesPortalSimpleApp.task.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {taskList.map((task, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`hcpNphiesPortalSimpleApp.TaskCodeEnum.${task.code}`} />
                  </td>
                  <td>{task.description}</td>
                  <td>{task.focus}</td>
                  <td>
                    <Translate contentKey={`hcpNphiesPortalSimpleApp.TaskReasonCodeEnum.${task.reasonCode}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="hcpNphiesPortalSimpleApp.task.home.notFound">No Tasks found</Translate>
            </div>
          )
        )}
      </div>
